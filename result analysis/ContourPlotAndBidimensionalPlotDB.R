...
axis3d('x',at=seq(1,length(x),10),labels ="" ,cex=0.6,col="black")
mtext3d(paste(rownames(finalMatrix)[myLabelX]), at=seq(1,length(x),10),edge='x', line=0.5,cex=0.6,col="black")
...
writeOBJ(paste(pathOutputPDF,"modelloDB.obj",sep = ""),withTextures = TRUE)
...
pdf(paste(pathOutputPDF,"andamentoMinPtsDB",".pdf",sep = ""))
...
text(myLabels1, par("usr")[3] - 0.1, labels = minPTS1[myLabels1], srt = 45, pos = 1, xpd = TRUE,cex = 0.7)
text(myLabels1, par("usr")[3] - 0.3, labels = minPTS2[myLabels1], srt = 45, pos = 1, xpd = TRUE,,cex = 0.7)
